...
from src.config import GOAL_OUTLIER_FLIGHTS_MAX
from src.config import GOAL_OUTLIER_KCALS_MAX
from src.config import GOAL_OUTLIER_KM_MAX
from src.config import GOAL_OUTLIER_STEPS_MAX
...
def _compute_goals(health_stats: list[dict]) -> dict[str, int]:
    """Compute goal dict from a list of health stats. Outliers above config thresholds are excluded.
    Averages: steps/kcals/km over all days (steps/km capped); flights over non-null only (capped).
    Rounded up: steps 1000, kcals 100, km and flights 1.
    """
    if not health_stats:
        return {"steps": 0, "kcals": 0, "km": 0, "flights_climbed": 0}

    steps_vals = [
        s.get("steps") or 0 for s in health_stats if (s.get("steps") or 0) <= GOAL_OUTLIER_STEPS_MAX
    ]
    steps_avg = sum(steps_vals) / len(steps_vals) if steps_vals else 0.0

    kcals_vals = [
        s.get("kcals") or 0 for s in health_stats if (s.get("kcals") or 0) <= GOAL_OUTLIER_KCALS_MAX
    ]
    kcals_avg = sum(kcals_vals) / len(kcals_vals) if kcals_vals else 0.0

    km_vals = [s.get("km") or 0 for s in health_stats if (s.get("km") or 0) <= GOAL_OUTLIER_KM_MAX]
    km_avg = sum(km_vals) / len(km_vals) if km_vals else 0.0

    flights_values = [
        s["flights_climbed"]
        for s in health_stats
        if s.get("flights_climbed") is not None and s["flights_climbed"] <= GOAL_OUTLIER_FLIGHTS_MAX
    ]
